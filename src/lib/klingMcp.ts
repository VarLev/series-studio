/**
 * Kling MCP — генерация на ПЛАТНЫХ КРЕДИТАХ ПОДПИСКИ Kling (официальный
 * хостед-сервер https://kling.ai/mcp, гайд kling.ai/app/mcp/guide).
 *
 * OAuth 2.1 — зеркально Higgsfield (проверено live-метаданными 2026-07-12):
 *  - issuer https://kling.ai/auth, DCR POST /auth/register (публичный клиент)
 *  - authorize /auth/authorize + PKCE S256, token /auth/token, refresh_token
 *  - scopes: generation.create generation.read account.credit.read
 * FAQ: цены = как на платформе Kling; бонусные кредиты и off-peak НЕ работают;
 * только Personal workspace; ссылки на результат живут 24 часа; отмены нет.
...
const MCP_URL = "https://kling.ai/mcp";
const AUTHORIZE_URL = "https://kling.ai/auth/authorize";
const TOKEN_URL = "https://kling.ai/auth/token";
const REGISTER_URL = "https://kling.ai/auth/register";
const SCOPE = "generation.create generation.read account.credit.read";

// ---------- хранение в settings ----------

/** Сеть до внешних API флапает из Node — первый fetch может упасть, повтор проходит. */
...
const K_CLIENT = "kling_mcp_client"; // {client_id, redirect_uri}
const K_TOKENS = "kling_mcp_tokens"; // {access_token, refresh_token?, expires_at}
const K_PENDING = "kling_mcp_pending"; // {verifier, state, redirect_uri}
...
async function ensureClient(redirectUri: string): Promise<ClientReg> {
  ...
  if (!res.ok) throw new Error(`Kling MCP: регистрация клиента не удалась (${res.status})`);
...
/** Шаг 1: URL авторизации — пользователь логинится в свой аккаунт Kling. */
...
async function tokenRequest(params: Record<string, string>): Promise<Tokens> {
  ...
  if (!res.ok || !body.access_token) {
    throw new Error(`Kling MCP: токен не выдан (${res.status} ${body.error ?? ""})`);
  }
  return {
...
export async function completeAuth(code: string, state: string): Promise<void> {
  ...
  if (!pending || pending.state !== state) {
    throw new Error("Kling MCP: state не совпал — начните подключение заново");
  }
  const client = await readKey<ClientReg>(K_CLIENT);
  if (!client) throw new Error("Kling MCP: клиент не зарегистрирован");
...
async function withMcp<T>(fn: (client: Client) => Promise<T>): Promise<T> {
  ...
  if (!token) throw new Error("Kling не подключён — нажмите «Подключить» в настройках");
...
export async function listKlingTools(): Promise<McpToolInfo[]> {
...
/**
 * Вызов инструмента. retry — ТОЛЬКО для идемпотентных вызовов (who_am_i,
 * query_tasks, query_membership_and_credits); генерации не ретраим,
 * чтобы не задвоить списание кредитов.
 */
export async function callKlingTool(
  name: string,
