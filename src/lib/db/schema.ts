...
export const shots = pgTable("shots", {
  ...
  // сквозное физическое состояние как ДИФ (JSON-массивы строк): факты, начавшиеся
  // в этой группе и живущие дальше (state_begin_json) / явно закончившиеся в ней
  // (state_end_json, дословные копии текста begin). Активное состояние группы НЕ
  // хранится — вычисляется свёрткой по связке сцены (carriedStateAtStart), поэтому
  // вставки/удаления/правки групп ничего не рассинхронизируют
  stateBeginJson: text("state_begin_json").notNull().default("[]"),
  stateEndJson: text("state_end_json").notNull().default("[]"),
  ...
  // снимок «первоначального состояния» группы для кнопки Revert: содержимое,
  // воссоздаваемое раскадровкой (title/beats/тайминг/тон/персонажи в кадре/якоря),
  // JSON GroupOriginSnapshot (см. lib/groupOrigin.ts). Фиксируется один раз при
  // создании группы; у групп до этой фичи — при первом открытии (текущее состояние).
  // null → снимок ещё не зафиксирован (Revert недоступен). Локация/погода в откат
  // НЕ входят — они едины на сюжетную связку, а не на группу.
