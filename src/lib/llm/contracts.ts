...
export const groupShotSchema = z.object({
  ...
  // id референсов ГРУППЫ (строки references с shotId+role), закреплённых за ЭТИМ
  // шотом как его вид/ракурс — drag-and-drop миниатюры на карточку шота. Привязка
  // живёт внутри объекта beat (как technique_id): переживает ренормализацию order
  // (retimeArea делает { ...b }); висячий id (референс откреплён) везде игнорируется
  ref_ids: z.array(z.string()).default([]),
