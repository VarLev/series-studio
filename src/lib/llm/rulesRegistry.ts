...
/**
 * Добавка для Enhance: та же грамматика полей, но УЖЕ описанный в шоте развивающий
 * кадр (developing shot — одна непрерывная траектория камеры, меняющая крупность/
 * объект внимания без склейки) неприкосновенен: Enhance его не режет и не упрощает
 * до статики. Инцидент 2026-07-19: непрерывный отъезд с детали кармана джинсов в
 * средний two-shot Enhance разрезал на два статичных шота, убив эффектное движение.
 * Запрет на неявный монтаж двух НЕсвязанных видов (инцидент эп.24) при этом остаётся.
 */
const VIEW_ENHANCE_ADDON = `Как это выглядит в твоём ответе:
* в полях framing и camera шота перечисляй, кто РЕАЛЬНО в кадре; в action описывай ТОЛЬКО происходящее в этом кадре;
* новый объект внимания, новый ракурс или новая крупность = НОВЫЙ шот — кроме исключения ниже;
* ИСКЛЮЧЕНИЕ — развивающийся шот (developing shot): если в шоте УЖЕ описана ОДНА непрерывная явная траектория камеры (наезд/отъезд, dolly, tilt, панорама, проводка), которая сама, без склейки, меняет крупность или объект внимания в одном непрерывном пространстве и времени — это ОДИН легитимный шот. НЕ разбивай его на несколько и НЕ упрощай до статичного кадра: сохрани описанное движение камеры от начала до конца;
* исключение НЕ отменяет запрет на два несвязанных вида («X смотрит, как вдали Y…»): между такими видами нет единой непрерывной траектории камеры — это по-прежнему ДВА шота.`;
...
export const SYSTEM_RULES: SystemRule[] = [
  ...
  },
  {
    ...
    description:
      "Каждая реплика отдельной строкой «[Имя]: -фраза», манера речи — маркером «[Имя](эмоция): -фраза». Процитированную сюжетом — дословно; сказанную косвенно («сказал что-то про идиота») модель пишет сама, а не пересказывает в действии.",
    text: DIALOGUE_RULES,
    usedIn: BREAKDOWN_SITES,
  },
  {
    ...
    textBySite: {
      ...
      enhance_group: VIEW_ENHANCE_ADDON,
  ...
  // ---------- Rework (правка группы по замечанию) ----------
  {
    ...
    text:
      ...
      "Маркер подачи «[Имя](эмоция):» — НЕ текст реплики: если запрос задаёт манеру речи («шепчет», " +
      "«кричит», «с издёвкой») — добавь или обнови маркер у соответствующей реплики, сам текст фразы " +
      "не меняя.",
    usedIn: ["revise_group"],
  },
  {
  ...
  // ---------- Enhance ----------
  {
    ...
    description:
      "Существующий сюжет/реплики и описанные движения камеры — источник истины; только шлифовка и дозаполнение полей.",
    text:
      ...
      "поправить оформление/пунктуацию и формат «[Имя]: -фраза»; существующий маркер подачи " +
      "«[Имя](эмоция):» сохраняй и не снимай — а проставить недостающий можно, см. список операций); " +
      "действие оставляй тем же по сути, " +
      "делая его точнее и кинематографичнее. " +
      "ОПИСАННОЕ ДВИЖЕНИЕ КАМЕРЫ — тоже источник истины: если в шоте уже задана непрерывная траектория " +
      "(наезд/отъезд, dolly, tilt, панорама, проводка) — сохрани её: НЕ упрощай такой шот до статичного " +
      "кадра, НЕ разрезай движение на несколько шотов и НЕ заменяй его другим движением; формулировку " +
      "можно уточнить, траекторию — нет.",
  ...
  },
  {
    ...
    text:
      ...
      "СУЩЕСТВУЮЩЕЕ содержимое между двумя шотами, ничего не выдумывая; НО разбиение не должно рвать " +
      "описанное непрерывное движение камеры (developing shot): такой шот оставь ЦЕЛЫМ с его траекторией, " +
      "а время выигрывай на других шотах; можно объединить два соседних " +
      "шота, если это не теряет содержания;\n" +
      "- уточнить тайминг каждого шота по формуле хронометража ниже;\n" +
      "- проставить маркер подачи «[Имя](эмоция): -фраза» (эмоция по-английски, 1–3 слова: whisper / " +
      "shouting / mocking / trembling / cold / tender) у реплик, чья манера произнесения следует из " +
      "действия шота или атмосферы сцены (шёпот вплотную, крик через комнату, издёвка) — текст фразы " +
      "не меняется; нейтральные реплики не размечай;\n" +
  ...
  {
    id: "prompt_dialogue_delivery",
    title: "Подача реплик (эмоция речи)",
    description:
      "Маркер «[Имя](эмоция):» из диалога шота — обязательная манера произнесения: шёпот остаётся шёпотом в тоне реплики и Performance (2026-07-19: «Джейкоб шепчет» терялся — подача нигде не фиксировалась).",
    text:
      "ПОДАЧА РЕПЛИК: в строках диалога шотов манера произнесения может быть задана маркером в скобках " +
      "сразу после имени говорящего — «[Имя](эмоция): -фраза» (напр. [Jacob](whisper): …). Такой маркер — " +
      "ОБЯЗАТЕЛЬНАЯ режиссёрская подача этой реплики: НЕ игнорируй, НЕ теряй и НЕ заменяй своей. " +
      "Перенеси её в тон реплики в промпте и согласуй с ней Performance и звук шота (громкость, дыхание, " +
      "мимика): шёпот, крик или издёвка меняют и звук, и картинку. У реплики без маркера подача как " +
      "обычно — по действию шота и эмоциональному тону группы.",
    textBySite: {
      shot_prompt_kling:
        "Формат Kling: тон — в скобках прямо в строке реплики: @Name says in English (whisper, " +
        'trembling): "Exact line."; в Audio отрази громкость (напр. hushed whisper, barely audible).',
      shot_prompt_seedance:
        "Формат Seedance: подачу реплики пиши в Performance соответствующего шота (напр. whispers, " +
        "barely audible / voice cracks) — текст фразы в DIALOGUE LOCK не меняется.",
      revise_prompt:
        "Если замечание пользователя задаёт манеру речи («шепчет», «кричит», «с издёвкой», «устало») — " +
        "обнови подачу соответствующей реплики: тон в строке Dialogue (says in English (…)) и/или " +
        "Performance шота, точный текст фразы сохрани.",
    },
