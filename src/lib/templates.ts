...
/**
 * Правила хронометража — применяются программно (как LANGUAGE_RULES), поверх
 * редактируемого шаблона, и при переделке группы по замечанию (llmReviseGroup).
 * В шаблон разбивки по умолчанию БОЛЬШЕ не вклеиваются: их копия там уходила в
 * модель вторым экземпляром и, главное, не снималась тумблером «Хронометраж
 * (формула)» на /rules — правило выключалось только наполовину.
 */
...
/**
 * Убрать блок хронометража из СОХРАНЁННОГО шаблона разбивки. Нужен, когда правило
 * «Хронометраж (формула)» выключено на /rules: программную копию снимает тумблер, а
 * копия внутри шаблона пользователя (старые сохранённые шаблоны делались из дефолта,
 * где блок был вклеен) иначе продолжала бы уходить в модель — тумблер не работал.
