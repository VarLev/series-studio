 ...
 *
 * Манера произнесения — необязательный маркер «[Имя](эмоция):». Появился
 * 2026-07-19: подача речи нигде не фиксировалась, и Rework «Джейкоб шепчет
 * Саймону» терял шёпот — записать его было некуда. Дальше по конвейеру маркер
 * подхватывает правило prompt_dialogue_delivery (rulesRegistry) — тон реплики
 * в видео-промпте.
 */
...
* манера произнесения (шёпот, крик, издёвка, дрожащий голос…) — маркером в скобках сразу после имени: «[Имя](эмоция): -фраза», эмоция по-английски, 1–3 слова: whisper / shouting / mocking / trembling / cold / tender. Ставь маркер, когда манеру задаёт сюжет или запрос пользователя («шепчет», «кричит», «цедит сквозь зубы») либо она однозначно следует из сцены; нейтральную реплику не размечай. Манера речи живёт ИМЕННО в этом маркере: «шепчет что-то» пересказом в action без маркера в dialogue — ОШИБКА. Маркер — не часть произносимой фразы, при расчёте времени по формуле его слова не считаются;
