 ...
 *
 * В шаблоне блок стоит НЕ случайно: он разделяет «не дроби простое действие» и
 * «Объедини шоты в группы не более 15 секунд», работая противовесом к «новую
 * группу начинай с нового действия, ракурса, реакции». Без него рядом с
 * группировкой остаётся только разрешение дробить — и модель отдаёт группы по
 * 3–4 секунды на одну реакцию (проверено на живом эпизоде 2026-07-17). Поэтому
 * копия в шаблоне и программная копия (R("timing_rules")) сосуществуют намеренно;
 * снимаются обе разом — тумблером «Хронометраж (формула)» на /rules, см.
 * stripTimingRules ниже.
 */
...
/**
 * Формат поля dialogue шота — применяется программно (как TIMING_RULES), чтобы
 * карточка шота показывала, КТО что говорит. В одном шоте может звучать диалог
 * нескольких персонажей: каждая реплика отдельной строкой «[Имя]: -фраза».
 *
 * Косвенная речь («сказал что-то про идиота на экране») — отдельный пункт не
 * случайно: раньше правило требовало «бери точный текст из сюжета», текста не
 * было, и модель оставляла dialogue пустым, а речь пересказывала в action.
 * Зритель такую реплику не слышит, а формула времени (estimateSpeechSeconds)
 * даёт пустой реплике 0 секунд — шот падает в «действие без слов, 2–4 сек», и
 * эпизод из одних бессловесных шотов физически не дотягивает до заказанного
 * хронометража (живой эпизод 25: 15 шотов → 58 сек при цели 1–2 минуты).
 */
export const DIALOGUE_RULES = `Формат реплик (поле dialogue каждого шота) — обязательно:
* если в шоте кто-то говорит, оформи КАЖДУЮ реплику отдельной строкой вида «[Имя]: -фраза»: в квадратных скобках — element_name говорящего (латиницей, как в библии), затем двоеточие, пробел, дефис и точный текст реплики на языке сюжета;
* когда в шоте несколько реплик (диалог двух и более персонажей) — каждая реплика с НОВОЙ СТРОКИ, в порядке произнесения (в JSON-строке перенос строки записывается как \\n);
* реплику, которую сюжет ПРИВОДИТ (прямой речью или в кавычках), бери ДОСЛОВНО и не переписывай; не сливай реплики разных персонажей в одну строку;
* если сюжет говорит о речи КОСВЕННО, не приводя слов («сказал что-то про идиота на экране», «отозвался коротко», «они о чём-то переговариваются»), — напиши реплику САМ: 4–12 слов, на языке сюжета, ровно о том, о чём сказано, в характере персонажа и тоне сцены. Это НЕ выдумывание сюжета: событие «он это сказал» уже есть в тексте, ты лишь даёшь ему слова. Оставить dialogue пустым, а речь пересказать в action («что-то говорит про идиота») — ОШИБКА: зритель должен УСЛЫШАТЬ реплику, а не прочитать о ней;
* если в шоте никто не говорит — dialogue это пустая строка.`;
...
/**
 * Убрать блок хронометража из шаблона разбивки. Нужен, когда правило «Хронометраж
 * (формула)» ВЫКЛЮЧЕНО на /rules: программную копию снимает сам тумблер, а копия
 * внутри шаблона иначе продолжала бы уходить в модель — правило выключалось только
 * наполовину (находка аудита). Пока правило включено, шаблон не трогаем вовсе.
 * Режем только ДОСЛОВНОЕ совпадение: переписанный пользователем текст — уже его
 * собственная инструкция, а не наше системное правило, и трогать его нельзя.
 */
export function stripTimingRules(tpl: string): string {
  if (!tpl.includes(TIMING_RULES)) return tpl;
  return tpl.replace(TIMING_RULES, "").replace(/\n{3,}/g, "\n\n");
}
