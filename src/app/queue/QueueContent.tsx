...
/**
 * Тело «Очереди» без экранной обвязки — общее для полной страницы (/queue) и
 * для правой панели (@panel/(.)queue). Ссылки на шот/референсы ведут на
 * основной экран: он не перехвачен, поэтому панель закрывается сама.
 */
export default async function QueueContent() {
  await requireAuth();
